...
files.forEach(file => {
  ...
  for (const [key, value] of Object.entries(replacements)) {
    // allow matching prefixes like dark: or hover: or focus:
    const regex = new RegExp(`(?<=[\\s"'\\\`:])(${key})(?=[\\s"'\\\`])`, 'g');
    
    // the regex allows : before the class. If it matches, it replaces the key with value.
    // However, if the key is `bg-navy`, and the string is `dark:bg-navy`, 
    // the match is `bg-navy` and it becomes `dark:bg-[#0B1F3A]`.
    content = content.replace(regex, value);
  }
  fs.writeFileSync(file, content);
  console.log(`Updated prefixes in ${file}`);
});
